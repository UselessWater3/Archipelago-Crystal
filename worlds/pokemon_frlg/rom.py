...
def write_tokens(world: "PokemonFRLGWorld") -> None:
    ...
    # /* 0x06 */ bool8 betterShops;
    # /* 0x07 */ bool8 cheaperCoins;
    # /* 0x08 */ bool8 reusableTms;
    # /* 0x09 */ bool8 unlockSeenDexInfo;
    # /* 0x0A */ bool8 physicalSpecialSplit;
    #
    ...
    # /* 0x2B */ u8 rematchRequiresGyms;
    #
    # /* 0x2C */ u32 startingMoney;
    #
    # /* 0x30 */ bool8 itemfinderRequired;
    # /* 0x31 */ bool8 flashRequired;
    # /* 0x32 */ bool8 fameCheckerRequired;
    # /* 0x33 */ bool8 bikeRequiresJumpingShoes;
    # /* 0x34 */ bool8 acrobaticBike;
    #
    # /* 0x35 */ u8 oaksAideRequiredCounts[5]; // Route 2, Route 10, Route 11, Route 16, Route 15
    #
    # /* 0x3A */ bool8 reccuringHiddenItems;
    # /* 0x3B */ bool8 isTrainersanity;
    # /* 0x3C */ bool8 isDexsanity;
    # /* 0x3D */ bool8 extraKeyItems;
    # /* 0x3E */ bool8 kantoOnly;
    # /* 0x3F */ bool8 flyUnlocks;
    # /* 0x40 */ bool8 isFamesanity;
    # /* 0x41 */ bool8 gymKeys;
    # /* 0x42 */ bool8 isShopsanity;
    #
    # /* 0x43 */ u8 removeBadgeRequirement; // Flash, Cut, Fly, Strength, Surf, Rock Smash, Waterfall
    # /* 0x44 */ u8 additionalDarkCaves; // Mt. Moon, Diglett's Cave, Victory Road
    #
    # /* 0x45 */ bool8 passesSplit;
    # /* 0x46 */ bool8 cardKeysSplit;
    # /* 0x47 */ bool8 teasSplit;
    #
    # /* 0x48 */ u8 startingLocation;
    # /* 0x49 */ u8 startingRespawn;
    # /* 0x4A */ u8 free_fly_id;
    # /* 0x4B */ u8 town_free_fly_id;
    # /* 0x4C */ u16 resortGorgeousMon;
    # /* 0x4E */ u16 introSpecies;
    # /* 0x50 */ u16 pcItemId;
    # /* 0x52 */ bool8 remoteItems;
    # /* 0x53 */ bool8 internalEntrancesRandomized;
    # /* 0x54 */ bool8 randomized;
    # /* 0x55 */ u8 version[16];
    # }
    ...
    patch.write_token(options_address, 0x07, struct.pack("<B", cheaper_coins))

    # Set reusable TMs and Move Tutors
    reusable_tm_tutors = 1 if world.options.reusable_tm_tutors else 0
    patch.write_token(options_address, 0x08, struct.pack("<B", reusable_tm_tutors))

    # Set unlock seen dex info
    all_pokemon_seen = 1 if world.options.all_pokemon_seen else 0
    patch.write_token(options_address, 0x09, struct.pack("<B", all_pokemon_seen))

    # Set physical/special split
    physical_special_split = 1 if world.options.physical_special_split else 0
    patch.write_token(options_address, 0x0A, struct.pack("<B", physical_special_split))
    ...
    patch.write_token(options_address, 0x0B, struct.pack("<B", open_viridian))

    # Set Pewter City roadblock
    route_3_condition = world.options.pewter_city_roadblock.value
    patch.write_token(options_address, 0x0C, struct.pack("<B", route_3_condition))
    ...
    patch.write_token(options_address, 0x1C, struct.pack("<B", block_vermilion_sailing))

    # Set all elevators locked
    all_elevators_locked = 1 if "All Elevators Locked" in world.options.modify_world_state.value else 0
    patch.write_token(options_address, 0x1D, struct.pack("<B", all_elevators_locked))
    ...
    patch.write_token(options_address, 0x2B, struct.pack("<B", rematch_requirements))

    # Set starting money
    patch.write_token(options_address, 0x2C, struct.pack("<I", world.options.starting_money.value))
    ...
    patch.write_token(options_address, 0x30, struct.pack("<B", itemfinder_required))

    # Set flash required
    flash_required = 1 if world.options.flash_required.value == FlashRequired.option_required else 0
    patch.write_token(options_address, 0x31, struct.pack("<B", flash_required))

    # Set fame checker required
    fame_checker_required = 1 if world.options.fame_checker_required else 0
    patch.write_token(options_address, 0x32, struct.pack("<B", fame_checker_required))
    ...
    patch.write_token(options_address, 0x33, struct.pack("<B", bicycle_requires_jumping_shoes))

    # Set acrobatic bicycle
    acrobatic_bicycle = 1 if world.options.acrobatic_bicycle else 0
    patch.write_token(options_address, 0x34, struct.pack("<B", acrobatic_bicycle))

    # Set Oak's Aides counts
    oaks_aide_route_2 = world.options.oaks_aide_route_2.value
    patch.write_token(options_address, 0x35, struct.pack("<B", oaks_aide_route_2))
    oaks_aide_route_10 = world.options.oaks_aide_route_10.value
    patch.write_token(options_address, 0x36, struct.pack("<B", oaks_aide_route_10))
    oaks_aide_route_11 = world.options.oaks_aide_route_11.value
    patch.write_token(options_address, 0x37, struct.pack("<B", oaks_aide_route_11))
    oaks_aide_route_16 = world.options.oaks_aide_route_16.value
    patch.write_token(options_address, 0x38, struct.pack("<B", oaks_aide_route_16))
    oaks_aide_route_15 = world.options.oaks_aide_route_15.value
    patch.write_token(options_address, 0x39, struct.pack("<B", oaks_aide_route_15))
    ...
    patch.write_token(options_address, 0x3A, struct.pack("<B", recurring_hidden_items))

    # Set trainersanity
    trainersanity = 1 if world.options.trainersanity.value != Trainersanity.special_range_names["none"] else 0
    patch.write_token(options_address, 0x3B, struct.pack("<B", trainersanity))

    # Set dexsanity
    dexsanity = 1 if world.options.dexsanity.value != Dexsanity.special_range_names["none"] else 0
    patch.write_token(options_address, 0x3C, struct.pack("<B", dexsanity))

    # Set extra key items
    extra_key_items = 1 if world.options.extra_key_items else 0
    patch.write_token(options_address, 0x3D, struct.pack("<B", extra_key_items))

    # Set kanto only
    kanto_only = 1 if world.options.kanto_only else 0
    patch.write_token(options_address, 0x3E, struct.pack("<B", kanto_only))
    ...
    patch.write_token(options_address, 0x3F, struct.pack("<B", fly_unlocks))

    # Set famesanity
    famesanity = 1 if world.options.famesanity else 0
    patch.write_token(options_address, 0x40, struct.pack("<B", famesanity))

    # Set gym keys
    gym_keys = 1 if world.options.gym_keys else 0
    patch.write_token(options_address, 0x41, struct.pack("<B", gym_keys))

    # Set shopsanity
    shopsanity = 1 if world.options.shopsanity else 0
    patch.write_token(options_address, 0x42, struct.pack("<B", shopsanity))
    ...
    patch.write_token(options_address, 0x43, struct.pack("<B", remove_badge_requirements))
    ...
    patch.write_token(options_address, 0x44, struct.pack("<B", additional_dark_caves))
    ...
    patch.write_token(options_address, 0x45, struct.pack("<B", passes_split))

    # Set card keys split
    card_keys_split = 1 if world.options.card_key.value in {CardKey.option_split, CardKey.option_progressive} else 0
    patch.write_token(options_address, 0x46, struct.pack("<B", card_keys_split))

    # Set teas split
    teas_split = 1 if world.options.split_teas else 0
    patch.write_token(options_address, 0x47, struct.pack("<B", teas_split))

    # Set starting town
    starting_town = data.constants[world.starting_town]
    patch.write_token(options_address, 0x48, struct.pack("<B", starting_town))

    # Set starting respawn
    starting_respawn = data.constants[world.starting_respawn]
    patch.write_token(options_address, 0x49, struct.pack("<B", starting_respawn))

    # Set free fly location
    patch.write_token(options_address, 0x4A, struct.pack("<B", world.free_fly_location_id))

    # Set town map fly location
    patch.write_token(options_address, 0x4B, struct.pack("<B", world.town_map_fly_location_id))

    # Set resort gorgeous mon
    patch.write_token(options_address, 0x4C, struct.pack("<H", world.logic.resort_gorgeous_pokemon))

    # Set intro species
    species_id = world.random.choice(list(data.species.keys()))
    patch.write_token(options_address, 0x4E, struct.pack("<H", species_id))
    ...
    if not world.options.remote_items and pc_item_location.item.player == world.player:
        ...
        item_id = data.constants["ITEM_ARCHIPELAGO_PROGRESSION"]
    patch.write_token(options_address, 0x50, struct.pack("<H", item_id))

    # Set remote items
    remote_items = 1 if world.options.remote_items else 0
    patch.write_token(options_address, 0x52, struct.pack("<B", remote_items))

    # Set interior ER
    shuffle_interiors = 1 if world.options.shuffle_interiors else 0
    patch.write_token(options_address, 0x53, struct.pack("<B", shuffle_interiors))

    # Set that the game has been randomized
    patch.write_token(options_address, 0x54, struct.pack("<B", 1))

    # Set apworld version
    apworld_version = f"AP v{world.world_version.as_simple_string()}"
    for j, b in enumerate(encode_string(apworld_version, 16)):
        patch.write_token(options_address, 0x55 + j, struct.pack("<B", b))
